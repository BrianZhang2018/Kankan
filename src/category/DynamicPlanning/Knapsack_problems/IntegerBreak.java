...
public class IntegerBreak{
    ...
    public int integerBreak1(int n) {
        ...
        for(int i=1; i<=n; i++){ // 问题的规模 (range)
            dp[i]=i;
            for(int j=1; j<=i; j++){
                dp[i]=Math.max(dp[i], dp[i-j]*j); // dp[i] means the maximum product value of i (i need to be divided into at least two positive parts)
...
// analysis for line 40:
// e.g. when i=2, j=5: dp[5] = max(dp[5], dp[3] * 2), 背包选择item的时候，要dp[5],还是要dp[3] * 2
