...
public class TheSkyline {
    public static void main(String[] args) {
        ...
        for (List<Integer> keyPoint : getSkyline(input))
            System.out.println(Arrays.toString(keyPoint.toArray()));
    }
    ...
    public static List<List<Integer>> getSkyline(int[][] buildings) {
        ...
        Collections.sort(heights, (a, b) -> {
            if (a[0] != b[0])
                ...
                return a[1] - b[1]; // Since the entering height is negative, so the larger abs(height) will be on
                                    // the front in max heap
        });
        ...
        for (int[] h : heights) {
            if (h[1] < 0) { // h[1] < 0, that means it meets a new building, so add it to pq
                pq.offer(-h[1]);
            } else { // h[1] >= 0, that means it has reached the end (right) of the building, so
                     // remove it from pq
                pq.remove(h[1]);
            }

            int currMax = pq.peek();
            if (currMax != prevMax) { // if the max height is different from the previous one, that means a critical
                                      // point is met, add to result list
         ...
         * }
         * int cur = tm.lastKey();
         * // compare current max height with previous max height, updateHotNodes result
         * and
