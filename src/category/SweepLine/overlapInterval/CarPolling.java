 ...
 * 2. Check overlap: compare the minimum end-time with current start-time, and
 * update the minimum end-time when need
 *
 * https://leetcode.com/problems/car-pooling/
 * 关键点就是 compare "previous end-time" with "start-time of current". 看有没有conflict,
 * 有的话就是累加
 *
...
public class CarPolling {
    public static void main(String[] args) {
        System.out.println(carPooling(
                new int[][] { { 2, 7, 8 }, { 1, 2, 3 }, { 3, 2, 3 }, { 1, 1, 7 }, { 9, 1, 2 }, { 10, 5, 6 } }, 12));
        System.out.println(carPoolingMap(new int[][] { { 2, 1, 5 }, { 3, 3, 7 } }, 4));
    }

    public static boolean carPooling(int[][] trips, int capacity) {
        Arrays.sort(trips, (a, b) -> a[1] - b[1]); // sort by start-time
        PriorityQueue<int[]> pq = new PriorityQueue<>((a, b) -> a[2] - b[2]); // compare the endTime, so keep the
                                                                              // minimum end-time on top
        for (int[] trip : trips) {
