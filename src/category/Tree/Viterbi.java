...
public class Viterbi {
    ...
    public int[] forwardViterbi(String[] inputs, String[] states, double[] sp,
                                double[][] tp, double[][] ep) {
        TNode[] T = new TNode[states.length];
        for (int i = 0; i < states.length; i++) {
            int[] intArray = new int[1];
            intArray[0] = i;
            T[i] = new TNode(intArray, sp[i] * ep[i][0]);
        }

        for (int output = 1; output < inputs.length; output++) {
            ...
            for (int next_state = 0; next_state < states.length; next_state++) {
                ...
                for (int state = 0; state < states.length; state++) {
                    ...
                    v_prob *= p;
                    if (v_prob > valmax) {
                        if (v_path.length == inputs.length) {
        ...
        for (int i = 0; i < argmax.length; i++) {
            System.out.print(Viterbi.states[argmax[i]] + ", ");
        }
